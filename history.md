...
- Implemented `shiftPointAlongRoute` in `src/mapViewport.mjs` and passed routing options from `src/app.js` to `buildMarkerDisplayState`. If overlap separation is triggered near endpoints, the truck point is now shifted back/forth *along the route polyline coordinates* instead of a direct flat longitude subtraction. This guarantees the truck always stays exactly on the road.
- Updated `callGhnDetail` in `src/trackingApi.mjs` and the Netlify function `track.js` to search the local `ghn_orders.json` database case-insensitively. This ensures that lowercase or mixed-case tracking codes (such as `ho1744510` vs `HO1744510`) correctly resolve and merge their order origin and destination coordinates.
- Implemented scan-event-based coordinates fallback in `generateRoutePoints()` inside `src/TrackingRouteManager.mjs`. If coordinates are not found in the order's metadata and the database merge fails, the map's start and end points automatically fall back to the first and last tracking scan log coordinates rather than default Hanoi and HCMC center coordinates.
